    ...
    <div class="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 text-gray-100">
        <div class="container mx-auto px-4 py-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                <div>
                    <h1 class="text-3xl font-bold text-white mb-2">Laporan Surat Keluar</h1>
                    <p class="text-gray-400">Kelola Laporan Surat</p>
                </div>
                <a href="{{ route('sekretaris.laporan-keluar.export') }}"
            ...
            <div class="bg-gray-800/50 rounded-xl p-5 border border-gray-700/50 mb-8 shadow-lg">
                <form action="{{ route('sekretaris.laporan-keluar') }}" method="GET">
                    <div class="flex flex-col md:flex-row items-end justify-between gap-4">
                        <div class="flex flex-col md:flex-row gap-6 w-full">
                            <!-- Filter Tanggal Surat Masuk -->
                            <div class="flex flex-col">
                                <label for="tgl_surat" class="text-sm font-medium text-gray-300 mb-1">Tanggal Surat
                                </label>
                                <input type="date" name="tgl_surat" id="tgl_surat" value="{{ request('tgl_surat') }}"
                        ...
                        <div class="flex gap-3">
                            ...
                            @if (request('no_surat') || request('tgl_surat'))
                                <a href="{{ route('sekretaris.laporan-masuk') }}"
                            ...
                            @forelse ($data as $index => $item)
                                <tr class="hover:bg-gray-700/30 transition-colors duration-150 text-center text-sm">
                                    ...
                                    <td class="py-3 px-4 text-gray-200">
                                        {{ $item->tgl_surat ? \Carbon\Carbon::parse($item->tgl_surat)->translatedFormat('d F Y') : \Carbon\Carbon::parse($item->tgl_dikirim)->translatedFormat('d F Y') }}
                                    </td>
                                    <td class="py-3 px-4 text-gray-200">
                                        {{ $item->tgl_dikirim ? \Carbon\Carbon::parse($item->tgl_dikirim)->translatedFormat('d F Y') : \Carbon\Carbon::parse($item->tgl_surat)->translatedFormat('d F Y') }}
                                    </td>
                                    ...
                                    <td class="py-3 px-4">
                                        <div class="flex gap-2">
                                            ...
                                            <a href="{{ route('sekretaris.suratkeluar.export', $item->no_surat) }}"
